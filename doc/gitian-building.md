...
- [Building Bunkercoin Core](#building-bunkercoin-core)
...
Clone the git repositories for bunkercoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bunkercoin-project/bunkercoin.git
git clone https://github.com/bunkercoin/gitian.sigs.git
```
...
in the bunkercoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Bunkercoin Core
----------------

To build Bunkercoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bunkercoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bunkercoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bunkercoin-project/bunkercoin
...
For example:
```bash
URL=https://github.com/laanwj/bunkercoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bunkercoin=${COMMIT} --url bunkercoin=${URL} ../bunkercoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bunkercoin=${COMMIT} --url bunkercoin=${URL} ../bunkercoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bunkercoin=${COMMIT} --url bunkercoin=${URL} ../bunkercoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bunkercoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bunkercoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bunkercoin
SIGPATH=/some/root/path/bunkercoin-detached-sigs

./bin/gbuild --url bunkercoin=${BTCPATH},signature=${SIGPATH} ../bunkercoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bunkercoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bunkercoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bunkercoin-osx-build.assert
```
...
[bunkercoin/gitian.sigs](https://github.com/bunkercoin-project/gitian.sigs/) repository, or if that's not possible create a pull
