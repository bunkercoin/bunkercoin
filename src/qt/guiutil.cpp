...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bunkercoin address (e.g. DJ7zB7c5BsB9UJLy1rKQtY7c6CQfGiaRLM)"));
#endif
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                ...
                if(!BitcoinUnits::parse(BitcoinUnits::BKC, i->second, &rv.amount, locale))
                {
                    return false;
                }
            }
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ...
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BKC, info.amount, false, true, localeC));
        paramCount++;
    }
...
{
    return GetAutostartDir() / "bunkercoin.desktop";
}
